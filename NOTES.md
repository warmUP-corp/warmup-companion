...
- Sign-in overlay keyboard: we never use Windows' gamepad OSK (TabTip / CoreInputView Gamepad). The companion draws `WarmupXboxVkWindow` (`WS_EX_TOPMOST | WS_EX_NOACTIVATE`) on the Winlogon desktop and injects keys with `SendInput` into the focused PIN field. While on Winlogon we disable TabletTip auto-invoke, stop `TextInputManagementService`, set `HKLM\SOFTWARE\Microsoft\Input\Settings\ControllerProcessor\ControllerToVKMapping\Enabled=0`, and hide/kill TabTip/TextInputHost so the native PIN keyboard cannot overlap the overlay. Opt back into the OS keyboard with `WARMUP_NATIVE_LOGON_VK=1` (`native_keyboard::yield_logon_to_native()`): then Xbox pads are driven by Windows and PlayStation/HID pads inject the on-screen PIN legend (`vk_nav::logon_pin_key`). That yield still applies only while `LogonUI.exe` owns the secure-desktop foreground; UAC keeps the overlay.
