...
- Sign-in ownership: Companion owns the L3 prompt and keyboard by default on every Windows build, for PIN and password entry. `WARMUP_NATIVE_LOGON_VK=1` explicitly enables the native-only LogonUI path; unset, `0`, and invalid values keep Companion ownership. Native mode retains the PlayStation PIN translation and Xbox OS handling, but all suppression entry points honor ownership so focus/injection cannot hide the native panel. UAC keeps Companion behavior. Resume resets readers, held input and cached credential focus; a five-second GetTickCount64 polling gap covers missed power broadcasts.
- Sign-in ownership layers (`native_keyboard::disable_auto_invoke`), applied on entering Winlogon and restored on return to userland: (1) device policy under `HKLM\SOFTWARE\Microsoft\PolicyManager\current\device\TextInput` — `TouchKeyboardControllerModeAvailability=2` (the 26100.4762 sign-in "Gamepad keyboard" is the touch keyboard's controller layout; 2 = always disabled), `EnableTouchKeyboardAutoInvokeInDesktopMode=0`, `AllowInputPanel=0`; (2) LogonUI profile `HKU\.DEFAULT\...\TabletTip\1.7` auto-invoke values; (3) `TabletInputService` Start=4 + live stop of `TextInputManagementService`; (4) the hide/kill sweep in `suppress()` as last resort. Priors are persisted to `C:\ProgramData\WarmupVk\signin-overrides.json` while live: a fresh process (crash, restart, `restore-keyboard`) restores from it, and a process that starts on Winlogon with the file present reuses the recorded priors instead of capturing its own overrides as "prior". After applying, one `sign-in ownership: OWNED|NOT GUARANTEED ...` line reads every value back and counts visible native keyboard windows; `warmup-companion.exe signin-report` prints the same line on demand.
