...
- Native sign-in keyboard gate: on Windows builds >= 26100.4762 (KB5062660) the OS ships its own controller-friendly sign-in keyboard, so we skip the TabletTip/`TextInputManagementService` suppression, the prompt card, and our VK on winlogon (`native_keyboard::native_logon_keyboard_available()`). Force with `WARMUP_NATIVE_LOGON_VK=1|0`. On a capable build we yield (`native_keyboard::yield_logon_to_native()`). Xbox (XInput) pads are driven by Windows itself; PlayStation/HID pads, which Windows cannot see at sign-in, instead get their buttons translated into the keystrokes of Windows' own on-screen PIN legend (`vk_nav::logon_pin_key`, injected via the Winlogon `SendInput` path) — the pad kind is fed each poll from `controller_label()` into `native_keyboard::set_logon_pad_is_xbox()`. The yield applies only while `LogonUI.exe` owns the secure-desktop foreground (the sign-in/lock surface, checked each poll via `xinput_backend::logon_credential_window()` + `native_keyboard::window_is_logonui()`); UAC consent and other secure prompts, where Windows has no controller keyboard, keep the old prompt/VK/suppression behavior. Every other secure-desktop edge is swallowed, so L3 cannot reopen our VK there.
