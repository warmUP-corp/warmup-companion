...
fn main() {
    ...
    let out_path = Path::new(&out_dir).join("predict_ngram.rs");
    fs::write(
        &out_path,
        emit_rust(&lexicon, &unigram, &bigram_rows, &trigram_packed),
    )
    .expect("write predict_ngram.rs");
}
...
fn emit_rust(
    ...
    s.push_str("// Leipzig Wortschatz word list + sentences (see assets/README.md)\n\n");
    s.push_str("pub static LEXICON: &[&str] = &[\n");
    for w in lexicon {
        s.push_str(&format!("    \"{}\",\n", escape(w)));
    }
    s.push_str("];\n\n");
    s.push_str(&format!(
        "pub static UNIGRAM: &[u8; {}] = &{:?};\n\n",
        unigram.len(),
        unigram
    ));

    emit_sparse_rows(&mut s, "BIGRAM", bigram_rows);
    emit_trigram(&mut s, trigram_packed);
    s.push_str(
        r#"
pub fn word_id(word: &str) -> Option<u16> {
    LEXICON.binary_search(&word).ok().map(|i| i as u16)
}

pub fn unigram_score(id: u16) -> u32 {
    UNIGRAM.get(id as usize).copied().unwrap_or(0) as u32
}

pub fn bigram_score(prev: u16, next: u16) -> u32 {
    let (row, scores) = match (
        BIGRAM_NEXT.get(prev as usize),
        BIGRAM_SCORE.get(prev as usize),
    ) {
        (Some(r), Some(s)) => (r, s),
        _ => return 0,
    };
...
pub fn trigram_score(w0: u16, w1: u16, next: u16) -> u32 {
    let i = match TRIGRAM_PAIR.binary_search_by_key(&(w0, w1), |p| (p.0, p.1)) {
        Ok(i) => i,
        Err(_) => return 0,
    };
    let (row, scores) = match (TRIGRAM_NEXT.get(i), TRIGRAM_SCORE.get(i)) {
        (Some(r), Some(s)) => (r, s),
        _ => return 0,
    };
    row.iter()
...
fn emit_sparse_rows(out: &mut String, prefix: &str, rows: &[Vec<(u16, u8)>]) {
    let mut all_next: Vec<Vec<u16>> = Vec::new();
    let mut all_score: Vec<Vec<u8>> = Vec::new();
    for row in rows {
        let (n, sc): (Vec<u16>, Vec<u8>) = row.iter().map(|(a, b)| (*a, *b)).unzip();
        all_next.push(n);
        all_score.push(sc);
    }
    out.push_str(&format!("pub static {}_NEXT: &[&[u16]] = &[\n", prefix));
    for row in &all_next {
        out.push_str(&format!("    &{:?},\n", row));
    }
    out.push_str("];\n\n");
    out.push_str(&format!("pub static {}_SCORE: &[&[u8]] = &[\n", prefix));
...
fn emit_trigram(out: &mut String, packed: &[(u16, u16, Vec<(u16, u8)>)]) {
    let pairs: Vec<(u16, u16)> = packed.iter().map(|(a, b, _)| (*a, *b)).collect();
    let mut next_rows = Vec::new();
    let mut score_rows = Vec::new();
    for (_, _, row) in packed {
        let (n, sc): (Vec<u16>, Vec<u8>) = row.iter().map(|(a, b)| (*a, *b)).unzip();
        next_rows.push(n);
        score_rows.push(sc);
    }
    out.push_str(&format!(
        "pub static TRIGRAM_PAIR: &[(u16, u16)] = &{:?};\n\n",
        pairs
    ));
    out.push_str("pub static TRIGRAM_NEXT: &[&[u16]] = &[\n");
    for row in &next_rows {
        out.push_str(&format!("    &{:?},\n", row));
    }
    out.push_str("];\n\n");
    out.push_str("pub static TRIGRAM_SCORE: &[&[u8]] = &[\n");
